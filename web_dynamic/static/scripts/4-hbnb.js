            ...
            success: function (place) {
                for (let l = 0; l < place.length; l++) {
                    $.get('http://0.0.0.0:5001/api/v1/places/' + place[l].id + '/amenities', function (place_amenities) {
                        ...
                        for (let k = 0; k < Object.keys(selectedAmenities).length; k++) {
                            for (let m = 0; m < place_amenities.length; m++) {
                                if (place_amenities[m].id === Object.keys(selectedAmenities)[k]) {
                                    amenities_count += 1;
                                }
                            }
                        }
                        if (amenities_count === Object.keys(selectedAmenities).length) {
